...
test.describe('Web Application E2E Tests', () => {
  ...
  test.describe('Landing Page', () => {
    ...
    test('should display network status', async ({ page }) => {
      ...
      await expect(status).toContainText(
        /Validator|Connected|online|offline|checking/i
      );
    });
  });
  ...
  test.describe('Wallet Connection', () => {
    test('should show wallet connection button', async ({ page }) => {
      const connectBtn = page.getByRole('button', {
        name: /connect wallet|install metamask|metamask/i,
      });
    ...
    test('should display wallet providers when modal open', async ({
      page,
    }) => {
      const connectBtn = page.getByRole('button', {
        name: /connect|connect wallet/i,
      });
      ...
      const hasAnyWallet = await page
        .getByText(/MetaMask|WalletConnect|Coinbase|Connect/)
        .isVisible();
  ...
  test.describe('RPC Endpoint Integration', () => {
    ...
    test('should display real-time block updates', async ({ page }) => {
      ...
      await page
        .locator('[data-testid="latest-block"]')
        .waitFor({ state: 'visible', timeout: 25000 })
        .catch(() => {});
      const initialBlock =
        (await page.locator('[data-testid="block-number"]').textContent()) ??
        '';
  ...
  test.describe('Transaction Flow', () => {
    test.skip('should show transaction form (no /send page)', async ({
      page,
    }) => {
  ...
  test.describe('Explorer Integration', () => {
    ...
    test('should display transaction details', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="tx-hash"]')).toContainText(
        testTxHash
      );
